...
local Window = Fluent:CreateWindow({
    Title = "Teste Shadow | Blox Fruits",
    SubTitle = "",
...
local TesteSelect = MainTab:AddDropdown("TesteSelect", {
    Title = "",
    Values = { "Fight Style", "Sword", "Gun" },
    Multi = false,
    Default = "selecione",
...
local function VoarParaTikiQuest3()
    local char = player.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return end

    local target = workspace:FindFirstChild("TikiQuest3")
    if not target then
        warn("TikiQuest3 não encontrado no workspace.")
        return
    end
    ...
    flyingConnection = RunService.RenderStepped:Connect(function()
        if not AutoFarmAtivo or not target or not hrp then
        ...
        if distance < 5 then
            flyingConnection:Disconnect()
            print("Chegou em TikiQuest3")

            -- Pressionar tecla com base na seleção
            if SelectedWeapon == "Fight Style" then
        ...
        hrp.CFrame = hrp.CFrame + direction * 2.5 -- velocidade ajustável
    end)
end

local TesteToggle = MainTab:AddToggle("TesteToggle", {
    Title = "Auto Farm",
    Default = false,
    Callback = function(Value)
        AutoFarmAtivo = Value
